...
function runtimeState(){const latest=runtimeEvents.at(-1);const sessionId=latest?.session_id??latest?.sessionId??'unknown-session';const sessionEvents=runtimeEvents.filter(event=>(event.session_id??event.sessionId??sessionId)===sessionId);const failed=sessionEvents.some(event=>event.type==='test_execution'&&event.success===false);const verified=sessionEvents.some(event=>event.type==='test_execution'&&event.success===true);const ended=sessionEvents.find(event=>event.type==='session_ended');return {sessionId,status:ended?(ended.success===false?'failed':'completed'):sessionEvents.length?'active':'idle',stage:ended?(ended.success===false?'failed':'verified'):failed?'failed':verified?'verified':'detected',eventCount:sessionEvents.length,lastEvent:latest??null,startedAt:runtimeStartedAt,updatedAt:latest?.timestamp??runtimeStartedAt,connectionStatus:clients.size?'CONNECTED':'DISCONNECTED',events:runtimeCollection()};}
